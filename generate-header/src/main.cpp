...
static std::string escapeForLiteral(const std::string& s) {
    ...
    out.reserve(s.size());
    for (char c : s) {
        if (c == '\\') out += "\\\\";  // becomes \\\\ in code => \\ at compile time
        else if (c == '"') out += "\\\"";
        else            out += c;
    }
    return out;
...
int main(int argc, char* argv[]) {
    ...
    while (std::getline(in, line)) {
        ...
        if (t.size() >= 2 && t.front() == '[' && t.back() == ']') {
            ...
            else if (tag == "RegistryKeys") sec = REGS;  // fixed typo
            else sec = NONE;
        }
            ...
            // Normalize doubled backslashes in input (e.g. "\\" -> "\\")
            std::string normalized;
            normalized.reserve(content.size());
            for (size_t i = 0; i < content.size(); ++i) {
                if (content[i] == '\\' && i + 1 < content.size() && content[i + 1] == '\\') {
                    normalized += '\\';
                    ++i; // skip the second \\ in the pair
                }
                else {
            ...
            if (sec == DIRS) dirs.push_back(normalized);
            if (sec == REGS) regs.push_back(normalized);
        }
    }
    in.close();
